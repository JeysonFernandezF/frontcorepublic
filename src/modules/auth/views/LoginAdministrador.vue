...
const {loading, errorLogin, login,logout,checkRoleAdmin} = useAuth();
...
const submit = async () => {
  await login(name.value,password.value, 'password');
  if(!checkRoleAdmin()){
    logout();
...
onMounted(()=>{
  showToast({message:"Inicia sesión con usuario administrador!",type:"success"})
})
...
<template>
  <section class="login-section">
    ...
    <div class="w-full text-center mt-2">
      <RouterLink :to="{name:'login'}"><IconPrc/>Ingresar con usuario indemin.</RouterLink>
    </div>
  </section>
</template>
